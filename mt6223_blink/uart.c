...
inline uint32_t UART_REG(int uart, enum uart_reg reg) {

	if (uart == 1) {
		return 0x80130000 + reg;
	} else if (uart == 2) {
		return 0x80180000 + reg;
	} else {
		return 0x801b0000 + reg;
	}
}
...
void uart_init(uint8_t uart, uint8_t interrupts) {
	...
	/* enable Tx/Rx FIFO, Tx trigger at 14 spaces, Rx trigger at 16 chars */
	//FIXME check those FIFO settings
	uart_reg_write(
			uart,
			FCR,
			FIFO_EN | RX_FIFO_CLEAR | TX_FIFO_CLEAR | (1 << TX_FIFO_TRIG_SHIFT)
					| (1 << RX_FIFO_TRIG_SHIFT));

	//uart_reg_write(uart, IIR, 0x47);

	/* Rx trigger */
	uart_reg_write(uart, RXTRIG, 0x0a);
...
void uart_putchar_wait(uint8_t uart, int c) {
	...
	{
	// wait while TX FIFO indicates full
	while (readw(MTK6223_DMA_FFSTA(13))  & MTK6223_FFSTA_FULL) {
	}
	writeb(c, MTK6223_DMA13_FIFO);
	}
	else
	{
		while (~readb(UART_REG(uart, LSR)) & 0x20) {
...
int uart_getchar(uint8_t uart) {
	...
	return readb(UART_REG(uart, RBR)) ;
...
static const uint16_t divider[] = { [UART_9600] = 339,[UART_19200] = 169,[UART_38400] = 85, [UART_57600] = 56,
		[UART_115200] = 28, [UART_230400] = 14, [UART_460800] = 7, [UART_921600
				] = 7, /* would need UART_REG(HIGHSPEED) = 1 */
};
#endif
...
int cons_putchar(char c) {
	...
	/* if the console is not active yet, we need to fall back */
	uart_putchar_wait(1, c);
	return 0;

}

void EnableVFIFO(uint8_t uart)
{
	// virtual FIFO enable
	writew(1,UART_REG(uart, VFIFO_EN));
	uart_reg_write(
			uart,
			FCR,
			FIFO_EN | RX_FIFO_CLEAR | TX_FIFO_CLEAR | (0 << TX_FIFO_TRIG_SHIFT)
					| (0 << RX_FIFO_TRIG_SHIFT) | DMA_MODE);
